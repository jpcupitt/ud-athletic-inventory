...
export default function StaffProfile() {
  ...
  const activeItems = member.issuedItems.filter((i) => !i.returned);
  const returnedItems = member.issuedItems.filter((i) => i.returned);
  const totalValue = activeItems.reduce((s, i) => s + i.qty * i.pricePerUnit, 0);

  function handleIssue(item: IssuedItem) {
    issueToStaff(member!.id, item);
  }

  function handleReturn(itemId: string, qty: number) {
    returnFromStaff(member!.id, itemId);
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex flex-col items-start gap-4 md:flex-row md:items-start md:gap-5">
          {member.photoUrl ? (
            ...
            <div className="w-16 h-16 rounded-full bg-gray-600 flex items-center justify-center text-white text-xl font-bold shrink-0">
              {member.firstName[0]}{member.lastName[0]}
            </div>
          )}
          <div className="flex-1">
            <h1 className="text-2xl font-bold text-gray-800">{member.firstName} {member.lastName}</h1>
            <p className="text-gray-500 mt-0.5">{member.title}</p>
        ...
        <dl className="mt-5 grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm pt-5 border-t border-gray-100">
          <div>
            <dt className="text-gray-400 text-xs uppercase tracking-wide">Staff ID</dt>
        ...
        {(member.shirtSize || member.shortsSize || member.shoeSize) && (
          <div className="mt-4 pt-4 border-t border-gray-100 flex gap-6 text-sm">
            {member.shirtSize && <div><span className="text-xs text-gray-400 uppercase tracking-wide">Shirt </span><span className="font-medium text-gray-800">{member.shirtSize}</span></div>}
            {member.shortsSize && <div><span className="text-xs text-gray-400 uppercase tracking-wide">Shorts </span><span className="font-medium text-gray-800">{member.shortsSize}</span></div>}
            {member.shoeSize && <div><span className="text-xs text-gray-400 uppercase tracking-wide">Shoe </span><span className="font-medium text-gray-800">{member.shoeSize}</span></div>}
          </div>
        )}
