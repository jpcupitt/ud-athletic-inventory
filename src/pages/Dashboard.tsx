...
export default function Dashboard() {
  ...
  return (
    <div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            {budgetView === 'overview' ? (
              <BarChart data={currentBudgetData} margin={{ top: 2, right: 4, left: 10, bottom: 20 }}>
                ...
                <Tooltip contentStyle={{ fontSize: 12 }} formatter={(v) => [`$${Number(v).toLocaleString()}`, '']} />
                <Bar dataKey="budgeted" name="Budgeted" fill="#DAEAF5" stroke="#00539F" strokeWidth={1} radius={[2, 2, 0, 0]} maxBarSize={30}>
                  <LabelList dataKey="budgeted" content={(props) => <BarLabel {...(props as any)} formatter={(v) => `$${(v / 1000).toFixed(0)}k`} insideColor="#003c71" />} />
                </Bar>
                <Bar dataKey="spent" name="Spent" fill="#00539F" radius={[2, 2, 0, 0]} maxBarSize={30}>
                  <LabelList dataKey="spent" content={(props) => <BarLabel {...(props as any)} formatter={(v) => `$${(v / 1000).toFixed(0)}k`} />} />
                </Bar>
              </BarChart>
            ) : (
            ...
            return (
              ...
              >
                ...
                <div className="shrink-0 text-right">
                  <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${order.status === 'submitted' ? 'bg-[#DAEAF5] text-[#00539F]' : 'bg-[#EEF4FB] text-[#003c71]'}`}>
                    {order.status}
                  </span>
                ...
                lowInventory.map((item) => (
                  <button key={item.id} onClick={() => { setShowLowInventory(false); navigate(`/inventory/${item.id}`); }} className="w-full text-left flex items-center gap-3 p-3 border-b border-gray-100 hover:bg-gray-50">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-gray-400 mt-0.5">Below reorder threshold</p>
                    </div>
                    ...
                    <tbody>
                      {quickSubmitOrder.lines.map((line, i) => (
                        <tr key={i} className="border-b border-gray-50">
                          ...
                          <td className="py-2 text-right font-medium" style={{ color: line.qtyOrdered - line.qtyReceived > 0 ? '#bdbdbd' : '#15803d' }}>
      ...
      {showOrderSubmitted && (
        <div className="fixed bottom-6 right-6 z-50 text-white px-5 py-3 rounded-lg shadow-xl flex items-center gap-2 text-sm font-medium" style={{ backgroundColor: '#003c71' }}>
